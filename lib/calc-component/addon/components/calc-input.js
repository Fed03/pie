...
// FIXME: must be refactored entirely
...
export default Ember.Component.extend(EKMixin, {
  ...
  opKey: Ember.on(keyDown(), function(event) {
    const match = /^Numpad((Add)|(Subtract)|(Multiply)|(Divide))$/.exec(
      getCode(event)
    );
    if (match) {
  ...
  equalsKey: Ember.on(keyDown("NumpadEnter"), function() {
    this.send("equals");
  }),
  ...
  actions: {
    ...
    equals() {
      ...
      this.sendAction("onEquals", inputValue);
      this.setProperties({
        lastOperator: "equals",
        waitingForOperand: true
      });
