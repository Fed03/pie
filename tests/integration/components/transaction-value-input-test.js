...
moduleForComponent(
  ...
  "Integration | Component | transaction value input",
  {
    integration: true
  }
);
...
test("it adds a class on click", async function(assert) {
  ...
  assert.notOk(
    findWithAssert(testSelector("transaction-value-input")).classList.contains(
      "input-focused"
    )
  );
...
// test("it opens an overlay on click", async function(assert) {
//   this.render(hbs`{{transaction-value-input}}`);
//
...
test("changing value updates the val", function(assert) {
  ...
  this.render(
    hbs`{{transaction-value-input transactionType=transactionType value=value}}`
  );
