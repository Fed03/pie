...
test("it renders", function(assert) {
  ...
  assert.ok(findWithAssert("[data-test-calculator]").classList.contains("calculator"), "It has the `calculator` class");
});

test("it displays the correct value", function(assert) {
  this.render(hbs`{{calc-input value=value}}`);
  assert.equal(findWithAssert("[data-test-calculator-display]").textContent.trim(), "0", "If value is undefined it starts at 0");
