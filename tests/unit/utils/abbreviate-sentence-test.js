import abbreviateSentence from "pie/utils/abbreviate-sentence";
import { module, test } from "qunit";

module("Unit | Utility | abbreviate sentence");

test("it returns the first letter and first consonant of a word", function(
  assert
) {
...
test("it returns only the first letter of a word if its length is 1", function(
  assert
) {
...
test("if the sentence has 2 words, it returns the first letter of every word", function(
  assert
) {
